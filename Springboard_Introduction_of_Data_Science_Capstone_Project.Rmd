...
#plot sum of earnings for top trade area
...
```{r, warning=FALSE}
# Model1 starts with fewer independent variables.
model1 <- lm(tcEquv_Act~vsl_type + TotalVoyageDays_Estimate_LatestDaySnapshot, v_mod_train)
...
model2 <- lm(tcEquv_Act~vsl_type + TotalVoyageDays_Estimate_LatestDaySnapshot + Cargo + Cargo_Lift_Act, v_mod_train)
...
model3 <- lm(tcEquv_Act~vsl_type + TotalVoyageDays_Estimate_LatestDaySnapshot + Cargo + Cargo_Lift_Act + Month + lastDiscPort + dwt, v_mod_train)
...
Earnings can be predicted using vessel type, loading and discharge ports, cargo type, deadweight tonnage, cargo lift these independent variables. Future analysis, weather related data and a model with different algorithms like Random Forest can help to improve the model.
